...
const ClientPortal = () => {
  ...
  return (
    <div className="min-h-screen bg-[#03031B] text-[#F8FAFC] font-body selection:bg-[#2B63EB]/30">
      <header className="border-b border-[#1E293B]/50 bg-[#03031B]/80 backdrop-blur-xl sticky top-0 z-10">
        <div className="max-w-5xl mx-auto flex items-center justify-between px-6 py-5">
          <div className="flex items-center gap-6">
            <img 
              src="/brand/logo-full.png" 
              alt="ClientFlow OS" 
              className="h-8 object-contain drop-shadow-[0_0_10px_rgba(43,99,235,0.3)]" 
            />
            <div className="h-6 w-px bg-[#1E293B]" />
            <div>
              <p className="font-extrabold text-[15px] tracking-tight text-white uppercase">{client?.name || "Client"}</p>
              <p className="text-[11px] text-[#64748B] font-bold tracking-widest uppercase">{client?.company || "Portal Access"}</p>
            </div>
          </div>
          <div className="hidden sm:flex items-center gap-2 px-4 py-1.5 bg-[#10B983]/10 border border-[#10B983]/20 rounded-full">
            <div className="w-1.5 h-1.5 rounded-full bg-[#10B983] animate-pulse" />
            <span className="text-[11px] font-bold text-[#10B983] tracking-wider uppercase">Live Connection</span>
          </div>
        </div>
      </header>
